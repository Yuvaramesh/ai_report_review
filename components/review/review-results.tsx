...
export default function ReviewResults({
  ...
  const queries = results && Array.isArray(results.queries) ? results.queries : [];
  const warnings = results && Array.isArray(results.warnings) ? results.warnings : [];
  const presentation = results && Array.isArray(results.presentation) ? results.presentation : [];
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const reviewData = {
        ...
        timestamp: timestamp,
        totalFindings: (errors?.length || 0) + (queries?.length || 0) + (presentation?.length || 0),
        uploadedFileNames: results?.uploadedFileNames || {},
      };
